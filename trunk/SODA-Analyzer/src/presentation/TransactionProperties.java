...
import applicationLogic.journal.trend.VolumeTrend;
...
/**
 * A simple Presentation class used to print the results of the transaction analysis.
 * The result of the computation is printed different txt files.
 * @author Staffiero
 *
 */
public class TransactionProperties {
	private static final String TRANSACTION = "tse";
	...
	public static void main(String[] args){
		try {
			...
			//Transaction tran = initializeTransaction("tse");
			//Transaction tran = initializeTransaction("tosem");
			
			Transaction tran = initializeTransaction(TRANSACTION);
			
			if (tran.isEmpty()){
				System.out.println("No Journals found for "+tran.getTransactionName());
				return;
			}
			
			System.out.println("Initialized transaction: "+tran.getTransactionName());
			
			
			File dir = new File(tran.getTransactionName());
			if (!dir.exists())
				dir.mkdir();
			
			File subD1 = new File(dir.getAbsolutePath()+"/VolumeTrend");
			if (!subD1.exists())
				subD1.mkdir();
			...
			File subD3 = new File(dir.getAbsolutePath()+"/Transaction");
			if (!subD3.exists())
				subD3.mkdir();
	
						
//			volumeTrendAnalysis(tran);
//			graphAnalysis(tran);
			transactionStatistics(tran);
	...
	/**
	 * Creates a new {@link Transaction} object with the given transaction short name.
	 * This method opens an Hiberante session which will be stored in the Transaction object for further queries.
	 * 
	 * @param Transaction the transaction short name
	 * @return the initialized Transaction object.
	 * @throws SodaHibernateException
	 * @throws IOException
	 */
	public static Transaction initializeTransaction(String Transaction) throws SodaHibernateException, IOException{
		//Create a session
		...
		Transaction tran = new Transaction(session, Transaction);
	...
	/**
	 * Performs a volume trend analysis of the given transaction using the methods defined in the {@link VolumeTrend} object.
	 * All the results are stored in txt files saved in the $APPLICATION_PATH/$TRANSACTION_NAME/VolumeTrend/ folder
	 * 
	 * @param tran the transaction to analyze.
	 * @throws SodaHibernateException
	 * @throws IOException
	 */
	public static void volumeTrendAnalysis(Transaction tran) throws SodaHibernateException, IOException{
		//Create a time trend analysis tool
		VolumeTrend vt = new VolumeTrend(tran);
		
		
		//AUTHORS TREND
		System.out.println("Calculating authors statistics");
		List<AuthorsStatistics> authorsTrend = 
			vt.getAuthorsVolumeTrend();
		System.out.println("Done");
		
		
		
		//Create a file and a buffer to store authors' statistics
		File output = new File(tran.getTransactionName()+"/VolumeTrend"+
				"/authors-trend.txt");
		if (output.exists())
			output.delete();
		...
		List<PublicationsStatistics> publicationsTrend = 
			vt.getPublicationsVolumeTrend();
		System.out.println("Done");
		
	
		output = new File(tran.getTransactionName()+"/VolumeTrend"+
		...
		//prints statistics
		buffer.append("Volume\t\tNumber of publications\n");
		...
		fw = new FileWriter(output);
		//prints statistics
		buffer.append("Volume\t\tProductivity\n");
		...
		prodTrend = 
			vt.getProductivityTrendActiveAuthors();
		System.out.println("Done");
		...
		List<GraphStatistics> graphTrend = 
			vt.getGraphStatisticsVolumeTrend();
		System.out.println("Done");
		...
		buffer.append("LCC = Largest Connected Component\n"+
				...
				"volume\t\tgraph Size\tconn. components\tsecond LCC\t\tLCC size\t\tLCC CPL\t\tLCC AVGCC\tLCC Diameter\n");
		...
		graphTrend = 
			vt.getIncrementalGraphStatistics();
		System.out.println("Done");
...
//		}
	...
	/**
	 * Performs an analysis on the co-authorship graph of the given transaction using the methods defined in the {@link GraphManager} object.
	 * All the results are stored in txt files saved in the $APPLICATION_PATH/$TRANSACTION_NAME/Graph/ folder
	 * 
	 * @param tran the transaction to analyze.
	 * @throws SodaHibernateException
	 * @throws IOException
	 */
	public static void graphAnalysis(Transaction tran) throws SodaHibernateException, IOException{
		//GRAPH CONSTRUCTION
		
		GraphManager gm = new GraphManager(tran);
		...
		output = new File(tran.getTransactionName()+"/Graph"+
	...
	/**
	 * Performs an analysis on the given transaction, extracting some general statistics, using the methods defined in the {@link Transaction} object itself.
	 * All the results are stored in txt files saved in the $APPLICATION_PATH/$TRANSACTION_NAME/Transaction/ folder
	 * 
	 * @param conf the conference to analyze.
	 * @throws SodaHibernateException
	 * @throws IOException
	 */
	public static void transactionStatistics(Transaction tran) throws SodaHibernateException, IOException{
		...
		List<Integer> authorsPublications= 
			tran.getAuthorsPublications();
		System.out.println("Done");
		
		
		
		File output = new File(tran.getTransactionName()+"/Transaction"+
		...
		List<Integer> publicationsAuthors= 
			tran.getPublicationsAuthors();
		System.out.println("Done");
		
		output = new File(tran.getTransactionName()+"/Transaction"+
		...
		List<Author> pubsNumber= 
			tran.getAuthorsPublicationsNumber();
		System.out.println("Done");
